...
def compute_avg_bleu(a_gold, a_pred):
    ...
    for key in a_gold:
        answer = a_gold[key]
        gts[key] = [
            " ".join(normalize_answer(a_gold[key], is_japanese_sentence(answer)))
        ]
        res[key] = [
            " ".join(normalize_answer(a_pred[key], is_japanese_sentence(answer)))
        ]
...
class Precision:
    def compute_score(self, gts: Dict[Any, List[str]], res: Dict[Any, List[str]]):
        ...
        assert isinstance(
            gts, dict
        ), "gts must be a dict where values are lists of strings"
        assert isinstance(
            res, dict
        ), "res must be a dict where values are lists of strings"
        scores = []
        for key in gts:
            gt = gts[key]
            r = res[key]
